...
def processMap(folder_path, xr_col, yr_col, tss1_col, tss2_col, tss3_col, heading_col, stbd_offset, port_offset):
    ...
    # Create scatter plots for each TSS coil
    plt.figure(figsize=(10, 6))
    scatter1 = plt.scatter(xr1_col, yr1_col, c=combined_df[tss1_col], cmap='viridis', marker='o')
    scatter2 = plt.scatter(combined_df[xr_col], combined_df[yr_col], c=combined_df[tss2_col], cmap='viridis', marker='o')
    scatter3 = plt.scatter(xr3_col, yr3_col, c=combined_df[tss3_col], cmap='viridis', marker='o')
...
# Example usage:
# processMap('path/to/folder', 'XR', 'YR', 'TSS1', 'TSS2', 'TSS3', 'Heading', stbd_offset, port_offset)
...
tk.Button(root, text="Show Plot", command=show_plot, font=font).grid(row=10, column=0, columnspan=3, pady=10)
tk.Button(root, text="Show Map", command=show_map, font=font).grid(row=11, column=0, columnspan=3, pady=10)
